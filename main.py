...
class Combat:
    ...
    def turn(self):
        ...
        #Le Player commence le combat
        if self.active_player and self.turn_number == 1:
            Console.print(f"[cyan]Tour 1 : Vous commencez en premier ![/cyan]")
            self.player_turn()   
        
        #L'adverdsaire commence le combat            
    ...
    def player_turn(self):
            ...
            if player_interact == '1':
                ...
                Console.print(f"Il reste {self.opponent.stat["health"]} PV à {self.opponent.name}.")
            ...
            #Le Player choisit d'ouvrir son Inventaire pour le REGARDER    
                ...
                #Menu qui permet de choisir un objet dans l'inventaire ou de revenir en arrière
                ...
                #Le Player choisit d'utiliser un objet de son inventaire
                if Menu_choice == '1' :
                    
                    #Ré-affichage le l'inventaire
                    self.player.show_inventory()
                    
                    #Si le Player possède au moins 1 objet dans l'inventaire 
                    if self.player.inventory:
                        item_id = Prompt.ask("Entrez le nom de l'objet à utiliser :", choices = [i for i in range(len(self.player.inventory))])
                        
                        #Appel de la méthode self.use_item de la class Entity
                        self.player.use_item(item_id)
                               
                    else:
                        console.print("[yellow]Votre inventaire est vide ![/yellow]")           

                #Le Player choisit de revenir au menu principal
                elif Menu_choice == '2' :
                    
                    #Rappel de la méthode pour réinitialiser le tour
